    ...
    <section class="space-y-4" aria-labelledby="credit-card-spending-heading">
        ...
        <div class="grid gap-4 md:grid-cols-3">
            @foreach ([['label' => 'Total Credit Card Spending', 'key' => 'total'], ['label' => 'Card Expenses', 'key' => 'expenseCount'], ['label' => 'Average Monthly Card Spending', 'key' => 'averageMonthly']] as $card)
                <div class="space-y-2 rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900"><flux:heading size="lg">{{ $card['label'] }}</flux:heading><p class="break-words text-2xl font-semibold tabular-nums">@if ($card['key'] === 'expenseCount'){{ $this->report[$card['key']] }}@else<x-money :currency="auth()->user()->currency" :amount="$this->report[$card['key']]" />@endif</p></div>
            @endforeach
        </div>
    </section>
