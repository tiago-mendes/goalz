    ...
    public function personalGoals(): Collection
    {
        return Goal::query()->personalOwnedBy(auth()->user())
            ->with([
                ...
                'milestoneAchievements:id,goal_id,milestone_percentage',
            ])
            ->orderByRaw("CASE status WHEN 'active' THEN 0 WHEN 'paused' THEN 1 WHEN 'completed' THEN 2 ELSE 3 END")
            ->orderByRaw('target_date IS NULL')
            ->orderBy('target_date')->orderBy('name')->orderBy('id')->get();
...
<section class="mx-auto w-full max-w-6xl space-y-8">
    ...
    <div class="grid gap-4 sm:grid-cols-5">
        <div class="space-y-2 rounded-xl border border-zinc-200 p-6 dark:border-zinc-700 sm:col-span-1"><flux:heading size="lg">Active Goals</flux:heading><p class="text-2xl font-semibold tabular-nums">{{ $this->activeGoals }}</p></div>
        <div class="space-y-2 rounded-xl border border-zinc-200 p-6 dark:border-zinc-700 sm:col-span-4"><flux:heading size="lg">Total Target</flux:heading><p class="break-words text-2xl font-semibold tabular-nums"><x-money :currency="auth()->user()->currency" :amount="$this->totalTarget" /></p><flux:text class="text-sm tabular-nums"><x-money :currency="auth()->user()->currency" :amount="$this->totalAllocated" /> / <x-money :currency="auth()->user()->currency" :amount="$this->totalTarget" /> · {{ $this->totalProgressPercentage }}%</flux:text><div class="h-2 overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-700" role="progressbar" aria-label="Total funding progress" aria-valuenow="{{ $this->totalVisualProgressPercentage }}" aria-valuemin="0" aria-valuemax="100"><div class="h-full rounded-full {{ GoalProgressColor::classes($this->totalProgressPercentage) }}" style="width: {{ $this->totalVisualProgressPercentage }}%"></div></div></div>
    </div>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                @foreach ($this->pendingInvitations as $invitation)
                    <article wire:key="goal-invitation-{{ $invitation->id }}" class="space-y-4 rounded-xl border border-zinc-200 p-5 dark:border-zinc-700">
                        ...
                        <dl class="grid grid-cols-[auto_auto] gap-x-3 gap-y-1 text-sm tabular-nums"><dt>Target</dt><dd class="text-right"><x-money :currency="auth()->user()->currency" :amount="$invitation->goal->target_amount" /></dd><dt>Target date</dt><dd class="text-right">{{ $invitation->goal->target_date?->format('M j, Y') ?? '—' }}</dd></dl>
            ...
            <table class="w-full min-w-[52rem] text-left text-sm"><caption class="sr-only">Goals you own</caption><thead class="bg-zinc-50 dark:bg-zinc-900"><tr><th scope="col" class="px-4 py-3">Name</th><th scope="col" class="px-4 py-3">Funding progress</th><th scope="col" class="px-4 py-3">Target Date</th><th scope="col" class="px-4 py-3">Status</th><th scope="col" class="px-4 py-3">Actions</th></tr></thead>
                <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                    @forelse ($this->personalGoals as $goal)
                        @php($fundingPlan = $goal->fundingPlan())
                        @php($latestMilestone = $goal->highestCurrentlyAchievedMilestone())
                        <tr wire:key="owned-goal-{{ $goal->id }}"><td class="px-4 py-3 font-medium">{{ $goal->name }}</td><td class="min-w-64 px-4 py-3"><div class="flex justify-between gap-3 tabular-nums"><span><x-money :currency="auth()->user()->currency" :amount="$goal->allocatedAmount()" /> / <x-money :currency="auth()->user()->currency" :amount="$goal->target_amount" /></span><span>{{ $goal->progressPercentage() }}%</span></div><div class="mt-2 h-2 overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-700" role="progressbar" aria-label="{{ $goal->name }} funding progress" aria-valuenow="{{ $goal->visualProgressPercentage() }}" aria-valuemin="0" aria-valuemax="100"><div class="h-full rounded-full {{ GoalProgressColor::classes($goal->progressPercentage()) }}" style="width: {{ $goal->visualProgressPercentage() }}%"></div></div>@include('pages.goals.partials.funding-plan', ['fundingPlan' => $fundingPlan])@if ($latestMilestone)<div class="mt-2"><flux:badge color="green">✓ {{ $latestMilestone->label() }} achieved</flux:badge></div>@endif</td><td class="whitespace-nowrap px-4 py-3">{{ $goal->target_date?->format('M j, Y') ?? '—' }}</td><td class="px-4 py-3"><flux:badge :color="$goal->status === GoalStatus::Active ? 'green' : ($goal->status === GoalStatus::Paused ? 'yellow' : 'zinc')">{{ $goal->status->label() }}</flux:badge></td>
        ...
        <div class="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-700">
            <table class="w-full min-w-[52rem] text-left text-sm"><caption class="sr-only">Goals shared with accepted members</caption><thead class="bg-zinc-50 dark:bg-zinc-900"><tr><th scope="col" class="px-4 py-3">Goal</th><th scope="col" class="px-4 py-3">Owner</th><th scope="col" class="px-4 py-3">Funding progress</th><th scope="col" class="px-4 py-3">Target Date</th><th scope="col" class="px-4 py-3">Status</th><th scope="col" class="px-4 py-3">Actions</th></tr></thead><tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                @forelse ($this->sharedGoals as $goal)
                    @php($fundingPlan = $goal->fundingPlan())
                    @php($latestMilestone = $goal->highestCurrentlyAchievedMilestone())
                    <tr wire:key="shared-goal-{{ $goal->id }}"><td class="px-4 py-3 font-medium">{{ $goal->name }}</td><td class="px-4 py-3">{{ $goal->owner->name }}</td><td class="min-w-64 px-4 py-3"><div class="flex justify-between gap-3 tabular-nums"><span><x-money :currency="auth()->user()->currency" :amount="$goal->allocatedAmount()" /> / <x-money :currency="auth()->user()->currency" :amount="$goal->target_amount" /></span><span>{{ $goal->progressPercentage() }}%</span></div><div class="mt-2 h-2 overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-700" role="progressbar" aria-label="{{ $goal->name }} funding progress" aria-valuenow="{{ $goal->visualProgressPercentage() }}" aria-valuemin="0" aria-valuemax="100"><div class="h-full rounded-full {{ GoalProgressColor::classes($goal->progressPercentage()) }}" style="width: {{ $goal->visualProgressPercentage() }}%"></div></div>@include('pages.goals.partials.funding-plan', ['fundingPlan' => $fundingPlan])@if ($latestMilestone)<div class="mt-2"><flux:badge color="green">✓ {{ $latestMilestone->label() }} achieved</flux:badge></div>@endif</td><td class="whitespace-nowrap px-4 py-3">{{ $goal->target_date?->format('M j, Y') ?? '—' }}</td><td class="px-4 py-3"><flux:badge :color="$goal->status === GoalStatus::Active ? 'green' : ($goal->status === GoalStatus::Paused ? 'yellow' : 'zinc')">{{ $goal->status->label() }}</flux:badge></td><td class="px-4 py-3">
